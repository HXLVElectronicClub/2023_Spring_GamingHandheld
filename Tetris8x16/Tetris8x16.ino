...
LedControl lc_1 = LedControl(LED1_DIN, LED1_CLK, LED1_CS, 1);
LedControl lc_2 = LedControl(LED2_DIN, LED2_CLK, LED2_CS, 1);
...
void updateColumn(int colnum)
{
    ...
    lc_1.setColumn(0,colnum,(screen[colnum] >> (lcRows / 2)) | lc0[colnum]);
    lc_2.setColumn(0,colnum,screen[colnum] | lc1[colnum]);
}
...
void splashScreen()
{
     ...
     {
         lc_1.setRow(0,rownum,up[rownum]);
         lc_2.setRow(0,rownum,down[rownum]);
     }
}
...
#ifdef SWAP_X_Y
        int dy = map(analogRead(JOY_VRX), 0, 4095, -2, 2);
#else
        int dy = map(analogRead(JOY_VRY), 0, 4095, -2, 2);
#endif
        dy = dy/2;
#ifdef REVERSE_Y
        dy = -(dy);
...
#ifdef SWAP_X_Y
        int dx = map(analogRead(JOY_VRY), 0, 4095, -2, 2);
#else
        int dx = map(analogRead(JOY_VRX), 0, 4095, -2, 2);
#endif
        dx = dx/2;
#ifdef REVERSE_X
        dx = -(dx);
...
void gameOver()
{
 ...
 for(int rownum = 8; rownum >= 0; rownum--)
     {
         lc_2.setRow(0,rownum,baixo[rownum]);
         delay(100);
     }
 
     for(int rownum = 8; rownum >= 0; rownum--)
     {
         lc_1.setRow(0,rownum,cima[rownum]);
         delay(100);
     }
